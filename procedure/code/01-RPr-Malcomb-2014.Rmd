...
```{r load-DHS-geog-data, eval=FALSE, fig.height = 2}
...
```{r clip-map-drought-exposure1, warning = F}
...
```{r clip-map-drought-exposure2, warning = F}
names(fl) <- "fl"
...
### Create Zonal Statistic for Traditional Authorities (TA).

Instead of the raster generated by Malcomb et al., we chose to calculate climate vulnerability by Traditional Authorities. Using the [CRediT](https://credit.niso.org/) taxonomy to attribute collaboration and contributions, Elise Chan's and Colman Bashore's work contributed to the development of the software and methods for the aggregation of the vulnerability grid into Traditional Authorities.

``` {r zonal stat for climate vulnerability TAs}
# Aggregate to administrative geography Note: the output is a vector that is
# forced to a SpatialPolygons object (not sf)
zonal <- aggregate(
  x = vulnerability_r, # rast
  by = ta_v, # vect
  FUN = mean)
...
tm_shape(zonal) + 
  tm_polygons(col="vulnerability", style="jenks", title="Mean Climate Vulnerability by TA", palette="YlOrRd", lwd=0.1, border.col="white") +
tm_layout(main.title = "Malawi Climate Vulnerability",main.title.size = 1.2, main.title.position = "left", frame = FALSE, legend.width=3, legend.text.size = .7, legend.title.size = 1, asp=0.9, legend.outside = TRUE, legend.outside.position = c('right'))
...
Our reproduction produced differing results from the original Malcomb el al. study. The difference in results between the original and the reproduction differ by location (Figure 5). 

In my addition to the reproduction study, I added an aggregation of climate vulnerability in Malawi into Traditional Authorities (TA). The aggregation of raster data into overlaying vectors, also known as a zonal statistic, utilizes the aggregate function from the stars R module. Aggregation enables the audience to better understand the inequality in climate vulnerability by political unit. The largest cluster of high vulnerability counties is located in the south of the country. This addition does not change the results of the existing reproduction study. It adds helpful visualizations of the data. Aggregating climate vulnerability to administrative regions clearly communicates and lowers the difficulty of comprehension of the study findings. 
